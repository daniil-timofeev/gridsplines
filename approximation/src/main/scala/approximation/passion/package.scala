...
package object passion{
  ...
  /** Завершает конструирование элемента матрицы интегро-нтерполяционного метода (умножая его на температуропроводность)
    ...
    *         `val w = -(a+w)- 1.0/tau)`, где `tau` - время шага, сек.
    */
  @inline
  ...
  final def iteration(time: Double, indices: Array[Int], lengths: Array[Int], upper: Array[Double],
                      ...
                      toPassion: Array[Array[Double]]): Unit = {
  ...
  @inline
  private final def del(c: Double, b: Double,
                        delta: Double): Double = c + b*delta

  @inline
  private final def lam(r: Double, b: Double,
                        lambda: Double, del: Double): Double = (r - b * lambda) / del


  @inline
  final def forwardFirst(a: Double, c: Double, time: Double,
                         vect: Double, res: Array[Double]): Unit = {
  ...
  @inline
  final def forwardUnit(a: Double, c: Double, time: Double,
                        vect: Double, res: Array[Double]): Unit = {
    forwardFirst(- (a + c) - 1.0 / time, c, vect, res)
  }
