...
class TwoDGridUnit extends mutable.Specification{
  ...
  val grid: TwoDGrid[Radial, Ortho] = TwoDGrid.apply[Radial, Ortho](leftX.toArray, xCoords.toArray, rightX.toArray, leftY.toArray, xCoords.toArray, rightY.toArray, conds, 1.0)

  for(i <- 1 to 100){
    grid.step(3600.0, leftX.map(x => 2.0))
    grid.step(3600.0, leftX.map(x => 2.0), )
  }
